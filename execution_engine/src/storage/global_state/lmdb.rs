...
mod tests {
    ...
    fn should_not_run_out_of_space() {
        ...
        // Write test pairs 0..RESIZE_POINT
        {
            ...
            for TestPair { key, value } in &test_pairs[..RESIZE_POINT] {
                match write::<_, _, _, LmdbTrieStore, error::Error>(
                    CorrelationId::new(),
                    &mut txn,
                    ...
                    Err(error) => panic!("should write pair but received error: {:?}", error),
                }
            }
        ...
        // Write test pairs RESIZE_POINT..TEST_PAIRS  - should panic with MapFull if the map is not
        // resized.
        {
            let mut txn = global_state.environment.create_read_write_txn().unwrap();
