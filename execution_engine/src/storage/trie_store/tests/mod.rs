...
fn create_data() -> Vec<TestData<Bytes, Bytes>> {
    ...
    let leaf_3_hash = Digest::hash(&leaf_3.try_to_vec().unwrap());
    ...
    let node_2_hash = Digest::hash(&node_2.try_to_vec().unwrap());
    ...
    let ext_node_hash = Digest::hash(&ext_node.try_to_vec().unwrap());
    ...
    let node_1_hash = Digest::hash(&node_1.try_to_vec().unwrap());
