...
fn gh_1470_call_contract_should_verify_group_access() {
    ...
    builder
        .exec_instrumented(instrumented!(call_contract_request))
        .commit();
    ...
    builder
        .exec_instrumented(instrumented!(call_versioned_contract_request))
        .commit();
...
//     builder.exec_instrumented(instrumented!(exec_request_1)).expect_success().commit();
...
//     builder.exec_instrumented(instrumented!(call_contract_request)).commit();
...
//     builder.exec_instrumented(instrumented!(call_versioned_contract_request)).commit();
...
fn should_wasm_transfer_after_major_bump() {
    ...
    builder
        .exec_instrumented(instrumented!(wasm_transfer))
        .expect_success()
        .commit();
}
