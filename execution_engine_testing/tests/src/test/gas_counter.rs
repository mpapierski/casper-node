...
use casper_execution_engine::{
    ...
    shared::wasm_engine::PreprocessingError,
};
use casper_types::{contracts::DEFAULT_ENTRY_POINT_NAME, runtime_args, Gas, RuntimeArgs, U512};

use crate::{test::regression::test_utils::make_gas_counter_overflow, wasm_utils::module_template};
...
fn should_correctly_measure_gas_for_opcodes() {
    ...
    let gas_cost = builder.last_exec_gas_cost().value_u64();
    let expected_cost: u64 = accounted_opcodes.clone().into_iter().map(u64::from).sum();

    // eprintln!("expected {} gas cost {}", expected_cost, gas_cost);
    assert_eq!(
        gas_cost, expected_cost,
        "expected {} gas cost {} accounted {:?}",
        expected_cost, gas_cost, accounted_opcodes,
    );
}

#[ignore]
#[test]
fn should_not_raise_gas_error_for_expected_amount() {
    let opcode_costs = DEFAULT_WASM_CONFIG.opcode_costs();
    ...
    let exec_request = {
        let deploy_item = DeployItemBuilder::new()
            ...
            .with_session_bytes(session_bytes, RuntimeArgs::default())
            .with_empty_payment_bytes(runtime_args! {
                ARG_AMOUNT => U512::from(expected_cost),
            })
            .with_authorization_keys(&[*DEFAULT_ACCOUNT_ADDR])
            .with_deploy_hash([42; 32])
            .build();
...
#[ignore]
#[test]
fn should_raise_gas_limit_when_exhausting_meter() {
    let opcode_costs = DEFAULT_WASM_CONFIG.opcode_costs();
    ...
    let exec_request = {
        let deploy_item = DeployItemBuilder::new()
            ...
            .with_session_bytes(session_bytes, RuntimeArgs::default())
            .with_empty_payment_bytes(runtime_args! {
                ARG_AMOUNT => U512::from(expected_cost - 1),
            })
            .with_authorization_keys(&[*DEFAULT_ACCOUNT_ADDR])
            .with_deploy_hash([42; 32])
            .build();
    ...
    let lhs = builder.get_error().expect("should have error");
    assert!(
        matches!(lhs, Error::Exec(execution::Error::GasLimit)),
        "{:?}",
        lhs,
    );
}
