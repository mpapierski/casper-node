...
fn should_raise_insufficient_payment_when_caller_lacks_minimum_balance() {
    ...
    let account_1_response = builder
        .exec_instrumented(instrumented!(account_1_request))
        .commit()
...
fn independent_standard_payments_should_not_write_the_same_keys() {
    ...
    builder
        .run_genesis(&PRODUCTION_RUN_GENESIS_REQUEST)
        .exec_instrumented(instrumented!(setup_exec_request))
        .expect_success()
        .commit();
    ...
    builder
        .exec_instrumented(instrumented!(exec_request_from_genesis))
        .expect_success()
        .commit()
        .exec_instrumented(instrumented!(exec_request_from_account_1))
        .expect_success()
        .commit();
