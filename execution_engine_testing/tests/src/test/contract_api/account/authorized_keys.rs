...
fn should_raise_deploy_authorization_failure() {
    ...
    builder
        .clear_results()
        .exec_instrumented(instrumented!(exec_request_5))
        .commit();
    ...
    builder
        .clear_results()
        .exec_instrumented(instrumented!(exec_request_6))
        .expect_success()
        .commit();
    ...
    builder
        .clear_results()
        .exec_instrumented(instrumented!(exec_request_7))
        .commit();
    ...
    builder
        .clear_results()
        .exec_instrumented(instrumented!(exec_request_8))
        .commit()
        .expect_success();
}
...
fn should_not_authorize_transfer_without_deploy_key_threshold() {
    ...
    builder
        .run_genesis(&PRODUCTION_RUN_GENESIS_REQUEST)
        // Reusing a test contract that would add new key
        .exec_instrumented(instrumented!(add_key_1_request))
        .expect_success()
        .commit();

    builder
        .exec_instrumented(instrumented!(add_key_2_request))
        .expect_success()
        .commit();

    builder
        .exec_instrumented(instrumented!(update_thresholds_request))
        .expect_success()
        .commit();
