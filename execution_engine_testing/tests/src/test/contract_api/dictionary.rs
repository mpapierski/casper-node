...
fn setup() -> (InMemoryWasmTestBuilder, ContractHash) {
    ...
    builder
        .exec_instrumented(instrumented!(install_contract_request))
        .commit()
        .expect_success();
...
fn should_modify_with_owned_access_rights() {
    ...
    builder
        .exec_instrumented(instrumented!(modify_write_request_1))
        .commit()
        .expect_success();
    ...
    builder
        .exec_instrumented(instrumented!(modify_write_request_2))
        .commit()
        .expect_success();
...
fn should_not_write_with_read_access_rights() {
    ...
    builder
        .exec_instrumented(instrumented!(call_request))
        .commit();
...
fn trigger() {
    {
        ...
        let call_request_2 = ExecuteRequestBuilder::standard(
            ACCOUNT_1_ADDR,
            DICTIONARY_CALL_WASM,
            runtime_args! {
                dictionary_call::ARG_OPERATION => dictionary_call::OP_READ,
        ...
        builder
            .exec_instrumented(instrumented!(call_request_1))
            .expect_success()
            .commit()
            .exec_instrumented(instrumented!(call_request_2))
            .expect_success()
            .commit();
    }
    eprintln!("---");
...
fn should_be_able_to_perform_dictionary_read() {
    ...
    builder
        .exec_instrumented(instrumented!(dictionary_session_call))
        .expect_success()
        .commit();
}
...
fn should_be_able_to_perform_read_from_key() {
    ...
    builder
        .exec_instrumented(instrumented!(read_from_key_session_call))
        .expect_success()
        .commit();
}
