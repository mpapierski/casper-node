...
use casper_types::{
    ...
    contracts::NamedKeys,
    CLValue, ProtocolVersion, PublicKey, SecretKey, StoredValue, URef, U512,
};
...
impl reactor::Reactor for Reactor {
    ...
    fn new(
        ...
        let chainspec = config.create_chainspec_for_test();
...
async fn run_deploy_acceptor_without_timeout(
    ...
    let mut runner: Runner<ConditionCheckReactor<Reactor>> =
        Runner::new(test_scenario.clone(), &mut rng).await.unwrap();
